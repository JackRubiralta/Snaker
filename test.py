...
def printHamiltonianCycle(cycle):
    ...
    for y in range(HEIGHT):
        ...
        for x in range(WIDTH):
            ...
            if (cycle[x + y * WIDTH] == LEFT) or ((x > 0) and (cycle[(x - 1) + y * WIDTH] == RIGHT)):
...
def generatePathToFood(hamiltonian_cycle):
    ...
    while (True):
        ...
        match hamiltonian_cycle[current_tile]:
            case -6: # UP
                next_tile = current_tile + UP
            
            case 6: # DOWN
                next_tile = current_tile + DOWN

            case -1: # LEFT
                next_tile = current_tile + LEFT

            case 1: # RIGHT
                next_tile = current_tile + RIGHT
        ...
        match snakes_hamiltonian_cycle[tile]:
            ...
            case 1: # RIGHT
                ...
                if edge_tile in grid and snakes_hamiltonian_cycle[edge_tile] == LEFT and edge_tile in path_to_food and (abs(edge_tile // WIDTH - tile // WIDTH) == 1):
                    candidates.append((tile, edge_tile))
                    #print((abs(edge_tile // WIDTH - tile // WIDTH)))
...
def main():
    ...
    while True:
        ...
        printHamiltonianCycle(snakes_hamiltonian_cycle)
        print(f'len = {len(generatePathToFood(snakes_hamiltonian_cycle))}')

        #print(f'{snakes_hamiltonian_cycle=}')
        #print(f'{snake.head=}')
        #print(f'{snake.body=}')
        #print(f'{snake.tail=}')
        #print(f'{snake.food=}')
